...
import { Building, MapPin, Video, SlidersHorizontal, Search } from "lucide-react";
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className="border-orange-500 text-orange-500 hover:bg-orange-500 hover:text-white flex items-center gap-2"
              >
            ...
            <DropdownMenuContent align="end" className="w-48">
              <DropdownMenuItem onClick={() => setSortBy("alphabetical")}>Alphabetical (A-Z)</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy("videos")}>Most Videos</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
        ...
        {filteredSpeakers.map((speaker) => (
          ...
          >
            <div className="w-20 h-20 flex-shrink-0 rounded-full overflow-hidden bg-gray-100">
              <Image src={speaker.image} alt={speaker.name} width={80} height={80} className="object-cover w-full h-full" />
            </div>
            <div className="flex flex-col">
              <h2 className="text-orange-600 font-bold text-base">{speaker.name}</h2>
      ...
      {visibleCount < sortedSpeakers.length && (
        <div className="text-center pt-4">
          <Button onClick={handleLoadMore} className="bg-green-700 hover:bg-green-800 text-white">
            Load More . . .
          </Button>
        </div>
      )}
